  ...
  const buttonCode = `
    const [isOpen, setIsOpen] = useState(false);
        ...
        <div>
            ...
            >
            <span className="text-lg dark:text-gray-100 font-medium">Tittle</span>
            <svg
                className={\`h-5 w-5 transition-transform dark:text-gray-100 duration-300
            ...
            <div
            className={\`px-5 pb-4 transition-all duration-300 dark:text-gray-100 \${isOpen
		...
		<div className="max-w-3xl mx-auto flex flex-col justify-center items-center space-y-3 space-x-4 mt-4">
			<div className="">
				<button
					...
					onClick={() => setIsOpen(!isOpen)}
				>
				<span className="text-md dark:text-gray-100 font-medium">Tittle</span>
				<svg
					className={`h-5 w-5 transition-transform duration-300 transform dark:text-gray-100 ${
					isOpen ? "rotate-180" : ""
					}`}
					xmlns="http://www.w3.org/2000/svg"
					fill="none"
				...
				<div
				className={`px-3 py-1 transition-all duration-300 dark:text-gray-100 ${
					isOpen
					? "max-h-screen opacity-100 border rounded-md"
					: "max-h-0 opacity-0 overflow-hidden"
				}`}
